...
def train(content_image, style_image, model, log_dict, style_weight):
    ...
    t2 = time.time()
    # save_tensor_to_file(style_image, "{}/style_features_st{}_c{}_step{}.png"
    #                    .format(dir_to_save, log_dict["content_index"], log_dict["style_label"], step))
...
if __name__ == "__main__":
    ...
    dir_to_save = "{}/results/cifar_wideresnet28_s2t/{}/{}/".format(args.log_dir,
                                                                       args.style_weight,
                                                                       args.corruption)
    if not os.path.exists(dir_to_save):
        os.makedirs(dir_to_save)
    ...
    corrupted = datasets.CIFAR10("{}/CIFAR10/".format(args.data_dir),
                                     train=False, download=False, transform=transform)

    # Style image comes from a clean dataset
    clean_dataset = CifarCorruptedDataset(base_c_path='{}/CIFAR-10-C/'.format(args.data_dir),
    ...
    for indx, (img, label) in enumerate(clean_dataset):
        if type(label) == torch.Tensor:
            label = label.item()
        clean_images[indx] = img
    ...
    for index in range(1000):  #:range(len(corrupted)):
